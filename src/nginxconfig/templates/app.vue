  ...
  <div class="all do-bulma">
    ...
    >
      <div class="columns is-multiline">
        <div
          :class="`column ${splitColumn ? 'is-half' : 'is-full'} is-full-touch`"
        >
...
export default {
  ...
  async mounted() {
    ...
    for (let i = 0; i < this.activeDomains.length; i++)
      this.addSiteEvent(i + 1, true);
  ...
  methods: {
    ...
    updateDiff(newConf, oldConf) {
      try {
        ...
        console.error(e);
        this.$data.confFilesOutput = Object.entries(newConf).map(
          ([name, content]) => {
